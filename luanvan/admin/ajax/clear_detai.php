<?php
    ...
    if(isset($_SESSION['idgv'])){
        if(isset($_GET['id_dangkydetai']) && isset($_GET['phanhoi'])){
            $id_dangkydetai = $_GET['id_dangkydetai'];
            $phanhoi = $_GET['phanhoi'];
            $idgv = $_SESSION['idgv'];
            // Fix SQL Injection
            $phanhoi = str_replace('\'','\\\'',$phanhoi);

            // Cập nhật dangkydetai được duyệt
            $sql = "UPDATE `dangkydetai` SET `trangthai`='2',phanhoi='$phanhoi' WHERE id = $id_dangkydetai";
